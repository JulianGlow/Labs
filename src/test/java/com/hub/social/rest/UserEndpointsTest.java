...
import com.hub.social.Application;
...
public class UserEndpointsTest {
    ...
    public void shouldCreate() throws Exception {
        ...
        mockMvc
                ...
                .andExpect(header().string("Location", format("http://localhost/v1/users/%s", dummyUser.getId())))
    ...
    public void shouldReject() throws Exception {
        ...
        mockMvc
           ...
           .andExpect(content().json(jsonForErrors("username.empty.message", "email.empty.message").toString()));
    }
    ...
    public void shouldPublish() throws Exception {
        ...
        mockMvc
           ...
           .andExpect(header().string("Location", format("http://localhost/v1/messages/%s", message.getId())));
    }
    ...
    public void shouldGetInbox() throws Exception {
        ...
        List<Message> content = asList(user.publish("Hello world"));
        Page<Message> page = new PageImpl<>(content.subList(p.getOffset(), p.getPageSize()), p, content.size());
    ...
    public static JSONObject jsonForUser(User user) {
        try {
            return new JSONObject().put(
                    ...
                    "_links", new JSONObject().put(
                       "self", new JSONObject().put(
                          "href", format("http://localhost/v1/users/%s", user.getId()))).put(
                       "user-messages", new JSONObject().put(
                          "href", format("http://localhost/v1/users/%s/messages", user.getId()))).put(
                       "inbox", new JSONObject().put(
                          "href", format("http://localhost/v1/users/%s/inbox", user.getId()))
                    ));
        } catch(JSONException e) {
           throw new RuntimeException(e);
        }
